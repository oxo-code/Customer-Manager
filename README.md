...
4. The backend now uses HTTPS with self-signed SSL certificates (auto-generated on first run).
5. Run the backend publicly on all interfaces: `python -m uvicorn backend.main:app --host 0.0.0.0 --port 8001 --ssl-keyfile backend/.local/key.pem --ssl-certfile backend/.local/cert.pem`
...
- The backend serves the built frontend from the `dist` folder on port `8001` via HTTPS.
- In production, open the app via `https://<server-ip>:8001` instead of Vite's preview port.
- The backend keeps the API routes under `/api` and serves the frontend at the root URL.
- SSL certificates are auto-generated in `backend/.local/` on first run.
...
- The repository ignores local databases, uploads, generated auth secrets, and SSL certificates.
...
- The application uses HTTPS with self-signed SSL certificates for local development.
- SSL certificates are automatically generated by `backend/generate_ssl.py` on first run.
- Certificates are stored in `backend/.local/key.pem` and `backend/.local/cert.pem`.
